...
Identifying cell types and subtypes from routine histopathology images is key to advancing our computational understanding of human disease. Current tile-based models can capture detailed nuclear appearances but often overlook the broader tissue context that influences a cell's function and identity. Moreover, existing human annotations are coarse-grained and inconsistently distributed across different studies, making it difficult to achieve fine-grained, cell subtype-level labeling.

To address these challenges, we introduce **NuClass**, a pathologist workflow-inspired framework for *cell-wise* multi-scale integration of both morphology and microenvironmental context. NuClass is composed of two main modules: **Path local**, which focuses on nuclear morphology from 224×224-pixel image crops, and **Path global**, which models the surrounding 1024×1024-pixel neighborhood. A learnable gating module fuses their per-cell probability outputs, adaptively balancing local detail and contextual cues. To promote complementary learning, we incorporate an uncertainty-guided objective that encourages the global path to focus on regions where the local path is uncertain, and we provide calibrated confidence estimates and Grad-CAM visualizations to make predictions more interpretable.
